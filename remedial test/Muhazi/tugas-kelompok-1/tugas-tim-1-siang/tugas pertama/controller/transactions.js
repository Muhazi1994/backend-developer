...
class Transaction {
  ...
  async getAllTransactions(req, res, next) {
    try {
      ...
      const data = await query(
        "SELECT t.id, g.name as goodName, g.price, c.name as customerName, s2.service, t.time, t.quantity, t.total FROM transactions t JOIN customers c ON t.id_customer=c.id JOIN goods g ON g.id=t.id_good JOIN suppliers s ON g.id_supplier=s.id JOIN services s2 on t.id_service=s2.id",
        []
      );
  ...
  async getDetailTransaction(req, res, next) {
    try {
      // Find all data in transactions
      const data = await query(
        "SELECT t.id, g.name as goodName, g.price, c.name as customerName, s2.service, t.time, t.quantity, t.total FROM transactions t JOIN customers c ON t.id_customer=c.id JOIN goods g ON g.id=t.id_good JOIN suppliers s ON g.id_supplier=s.id JOIN services s2 on t.id_service=s2.id WHERE t.id=?",
        [req.params.id]
      );
  ...
  async createTransaction(req, res, next) {
    try {
      ...
      const insertedData = await query(
        "INSERT INTO transactions(id_customer, id_good, id_service, quantity, total) VALUES (?, ?, ?, ?, ?)",
        [
  ...
  async updateTransaction(req, res, next) {
    try {
      ...
      const updateData = await query(
        "UPDATE transactions SET id_good=?, id_customer=?, id_service=?, quantity=?, total=? WHERE id=?",
        [
          req.body.id_good,
          req.body.id_customer,
          req.body.id_service,
