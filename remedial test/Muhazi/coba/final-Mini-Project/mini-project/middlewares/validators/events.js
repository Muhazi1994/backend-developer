...
exports.createOrUpadateEventValidator = async (req, res, next) => {
  try {
    ...
    if (!(req.files && req.files.photoEvent)) {
      ...
      req.body.photoEvent = `https://timcevent.herokuapp.com/images/events/${file.name}`;
    }
    ...
    if (!(req.files && req.files.speakerPhoto)) {
      ...
      // assign req.body.image with file.name
      req.body.speakerPhoto = `https://timcevent.herokuapp.com/images/speakerPhoto/${file.name}`;
    }
