...
const fetchSembarangan = async () => {
    try {
        const { data: dataSembarangan } = await axios.get('https://bobsam.bobysamuel.repl.co/sembarang')
        const { data: anotherSembarang } = await axios.post('https://bobsam.bobysamuel.repl.co/sembarang', JSON.stringify({dataSembarangan}))
