  ...
  async getAllOrderMenu(req, res, next) {
    try {
      let data = await orderMenu.findAll({
        attributes: {
          exclude: ["menuId", "orderId", "createdAt", "updatedAt", "deletedAt"],
        },
        include: [
          {
            model: menu,
      ...
      if (data.length === 0) {
        return res.status(404).json({ errors: ["History Not Found!"] });
      }
            ...
            include: [
              {
                ...
                attributes: {
                  exclude: ["id","menuId","createdAt", "updatedAt", "deletedAt"],
                },
                include: [
                  {
                    model: variantOption,
                    where: { id: req.body.variantOptionId },
                    attributes: {
                      exclude: ["id","variantId","createdAt", "updatedAt", "deletedAt"],
                    },
                  },
                ],
              },
