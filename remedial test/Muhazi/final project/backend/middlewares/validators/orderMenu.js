...
exports.updateOrderMenuValidator = async (req, res, next) => {
  try {
    ...
    const findMenu = await menu.findOne({ where: { id: findOrderMenu.dataValues.menuId } });
    if (!findMenu) {
      errors.push("Menu does not exist");
    }
    ...
    if (specialPrice !== null) {
      req.body.subTotalPrice = parseFloat(req.body.quantity) * parseFloat(specialPrice);
    } else {
      req.body.subTotalPrice = parseFloat(req.body.quantity) * parseFloat(price);
    }
